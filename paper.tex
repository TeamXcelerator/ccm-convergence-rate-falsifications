...
\texttt{rayon} (v1.10) for parallelism. Root-finding uses
Halley's method (cubic convergence) as the default, with
analytically available first and second derivatives of the
rational function $R(z)$; the solver returns a three-state
result (\texttt{Converged}, \texttt{Approximate}, \texttt{Failed})
so that precision-floor stagnation is distinguished from true
convergence failure. Three subcommands
